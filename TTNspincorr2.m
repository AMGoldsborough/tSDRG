...
% Andrew Goldsborough 17/05/2013
    ...
    for site2 = site1+1:L
        ...
        for pathstep = stepprev:idxL-1
            ...
            if legL(pathstep) == 0
                ...
                coroL = tcon(coroL,w{i},[1,-2,-4],[-1,1,-3]);
                coroL = tcon(coroL,eyet,[-1,-2,1,2],[1,-3,2,-4]);
                coroL = tcon(coroL,conj(w{i}),[-1,1,2,-3],[-2,1,2]);
            else
                ...
                coroL = tcon(coroL,w{i},[1,-3,-4],[-1,-2,1]);
                coroL = tcon(coroL,conj(w{i}),[-1,1,2,-3],[-2,1,2]);
            end
        end
        ...
        for pathstep = 1:idxR-1
            ...
            if legR(pathstep) == 0
                ...
                coroR = tcon(coroR,w{i},[1,-2,-4],[-1,1,-3]);
                coroR = tcon(coroR,conj(w{i}),[-1,1,2,-3],[-2,1,2]);
            else
                ...
                coroR = tcon(coroR,w{i},[1,-3,-4],[-1,-2,1]);
                coroR = tcon(coroR,eyet,[-1,1,-3,2],[1,-2,-4,2]);
                coroR = tcon(coroR,conj(w{i}),[-1,1,2,-3],[-2,1,2]);
            end
        end
