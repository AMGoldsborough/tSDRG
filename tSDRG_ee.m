function tSDRG_ee(L,Jstr,Jdis,Jz,m,Pdist,Jseed)
% Tensor network SDRG algroithm calculating correlation functions and 
% entanglement entropy for bipartitions
% Keeps only full SU2 blocks

% Andrew Goldsborough 15/08/2013
...
fprintf('printing entanglement entropy\n');

%loop over the bipartition positions (splitting chain into two)
% AAAAA/BBBBB
%output format [spos ee n_a chi]
%where spos = position of bipartition, ee = entanglement entropy, n_a =
%number of legs connecting the two blocks, chi = product of the leg sizes
for spos = 1:L-1
    
    %create indices for which parts are A(1) and which are B(0)
    blocks = [2*ones(spos,1);zeros(L-spos,1)];
    
    %calculate ee by diagonalising a reduced density matrix
    try
        [ee,n_a,chi] = TTNeeDM2(L,w,blocks,Jorder,tL,tR);
        fprintf(fidee,'%d %.15e %d %d\n',spos, ee, n_a, chi);
    catch err
        continue
    end
end
